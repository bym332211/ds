...
def execute_trade(signal_data: dict, price_data: dict):
    ...
    try:
        # position sizing with ATR; risk <= equity * risk_fraction
        balance = exchange.fetch_balance()
        usdt_balance = balance['USDT'].get('free', 0)
        usdt_total = balance['USDT'].get('total', usdt_balance) if isinstance(balance['USDT'], dict) else usdt_balance
        equity = usdt_total or usdt_balance
        atr = float(price_data.get('technical_data', {}).get('atr', 0) or 0)
        atr_mult = float(TRADE_CONFIG.get('atr_multiplier', 1.5))
        effective_stop = max(risk, atr * atr_mult)
        risk_cap = float(TRADE_CONFIG.get('risk_fraction', 0.02)) * float(equity)
        trade_amount = risk_cap / effective_stop if effective_stop > 0 else 0
        # cap by margin availability (80% of free)
        entry_price = entry
        max_amount_by_margin = (usdt_balance * 0.8) * max(TRADE_CONFIG['leverage'], 1) / max(entry_price, 1e-9)
        trade_amount = max(0.0, min(trade_amount, max_amount_by_margin))
        # round to 6 decimals (BTC step)
        trade_amount = float(f"{trade_amount:.6f}")
        print(f"ATR: {atr:.2f}, 有效止损: {effective_stop:.2f}, 风险上限: {risk_cap:.2f}USDT, 下单数量: {trade_amount} BTC")
        if trade_amount <= 0:
            print("计算后的下单数量为0，跳过下单")
            return
